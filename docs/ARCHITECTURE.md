...
The risk score begins at 100 and is reduced by simulation performance and training completion. Simulation performance contributes 80 percent and training completion contributes 20 percent. Report labels are Low below 25, Moderate below 55, and High at 55 or above.

## Security and Privacy

No credentials or personal data are requested. Links shown inside simulated malicious content are non-navigable or use reserved `.example` domains. User state never leaves the browser. CDN dependencies are version-pinned; high-assurance deployments can self-host these files.
