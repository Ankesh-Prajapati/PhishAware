(function () {
  ...
  function initializeSimulation() {
    ...
    function renderQuestion() {
      ...
      document.getElementById('artifactStage').innerHTML = renderArtifact(scenarioId, question);
      bindClues();
      document.dispatchEvent(new CustomEvent('pa:artifact-rendered', { detail: { scenarioId, variant: question } }));
  ...
  function renderArtifact(scenarioId, variant) {
    ...
    if (scenarioId === 'social') {
      ...
      return `${safety}<div class="social-phone"><div class="social-top"><strong>Social</strong><div><i class="bi bi-heart"></i><i class="bi bi-send"></i></div></div><div class="social-account"><span class="social-avatar"><i class="bi bi-gift"></i></span><div><strong>${f.handle}</strong><div class="small clue" data-clue="${c.age}">${f.age} &middot; ${f.followers}</div></div><i class="bi bi-three-dots ms-auto"></i></div><div class="social-visual"><i class="bi bi-gift-fill"></i><strong class="clue" data-clue="${c.prize}">${f.prizeLabel}</strong><span>${f.prizeSub}</span></div><div class="social-actions"><i class="bi bi-heart"></i><i class="bi bi-chat"></i><i class="bi bi-send"></i><i class="bi bi-bookmark ms-auto"></i></div><div class="social-copy"><strong>${f.handle}</strong> ${body}</div><button type="button" class="social-claim">Claim now</button></div>`;
    }
