(function () {
  ...
  function renderArtifact(id) {
    ...
    if (id === 'bec') return `${safety}<div class="outlook-window"><div class="outlook-bar"><span class="outlook-app"><i class="bi bi-grid-3x3-gap-fill"></i> Outlook</span><div class="outlook-search"><i class="bi bi-search"></i> Search</div><span><i class="bi bi-camera-video"></i> Meet Now</span><i class="bi bi-gear"></i></div><div class="outlook-body"><aside><button type="button"><i class="bi bi-envelope-plus"></i> New mail</button><strong>Favorites</strong><span><i class="bi bi-inbox-fill"></i> Inbox <b>2</b></span><span><i class="bi bi-send"></i> Sent Items</span><span><i class="bi bi-file-earmark"></i> Drafts</span></aside><section class="outlook-message"><div class="outlook-actions"><span><i class="bi bi-reply"></i> Reply</span><span><i class="bi bi-arrow-90deg-right"></i> Forward</span><span><i class="bi bi-trash"></i> Delete</span><span><i class="bi bi-flag"></i> Report</span></div><h2>Confidential acquisition payment</h2><div class="outlook-sender"><span class="sender-avatar executive">AM</span><div><strong>Arjun Mehta - Chief Executive Officer</strong><div class="small">${clue('arjun.mehta@northstar-holdlngs.example', scenario.clues[3])}</div><div class="small text-muted">To: Accounts Payable</div></div></div><div class="outlook-copy"><p>Hi,</p><p>I am finalizing a confidential acquisition and need you to send <strong>INR 8,40,000</strong> to the attached <span class="clue" data-clue="${scenario.clues[2]}">new beneficiary account</span>.</p><p><span class="clue" data-clue="${scenario.clues[0]}">Do not discuss this with anyone</span>. Complete it <span class="clue" data-clue="${scenario.clues[1]}">before the normal approval meeting</span>. I am unavailable by phone.</p><p>Regards,<br>Arjun<br><small>Sent from my phone</small></p></div></section></div></div>`;

    if (id === 'mfa-fatigue') return `${safety}<div class="mfa-phone phone-frame"><div class="phone-status"><strong>9:41</strong><span><i class="bi bi-reception-4"></i> <i class="bi bi-wifi"></i> <i class="bi bi-battery-full"></i></span></div><div class="mfa-lockscreen"><div class="mfa-clock">02:17 AM</div><div class="mfa-date">Thursday</div><div class="mfa-stack">${[1,2,3,4,5].map((n,i)=>`<div class="mfa-push ${i===4?'mfa-push-top':''}"><span class="mfa-push-icon"><i class="bi bi-shield-lock-fill"></i></span><div class="mfa-push-body"><strong>Approve sign-in?</strong><span>${i===4?clue('New York, US &middot; Unknown device',scenario.clues[1]):'New York, US &middot; Unknown device'}</span></div><span class="mfa-push-time">${i===4?clue('Now',scenario.clues[3]):`${(5-i)*6}s ago`}</span></div>`).join('')}</div></div><div class="mfa-chat"><div class="mfa-chat-bubble"><span class="mfa-chat-avatar"><i class="bi bi-headset"></i></span><p><strong>"IT Support"</strong> (unknown number): ${clue('We see repeated login errors on your account, just approve the next request so we can clear it', scenario.clues[2])}</p></div><p class="small text-muted clue" data-clue="${scenario.clues[0]}">You did not attempt to sign in tonight</p></div></div>`;

    if (id === 'upi-fraud') return `${safety}<div class="upi-phone phone-frame"><div class="phone-status"><strong>9:41</strong><span><i class="bi bi-reception-4"></i> <i class="bi bi-wifi"></i> <i class="bi bi-battery-full"></i></span></div><div class="call-screen"><div class="call-avatar"><i class="bi bi-bank2"></i></div><div class="call-name">Bank Fraud Helpline</div><div class="call-number">+91 98xxxxxx12</div><div class="call-status">Incoming call &middot; 02:14</div><div class="call-transcript"><p class="clue" data-clue="${scenario.clues[0]}">"I can see your \u20b924,999 payment to Amazon failed yesterday."</p><p class="clue" data-clue="${scenario.clues[1]}">"Read me the OTP you just received so I can reverse it."</p></div><div class="call-actions"><span class="call-decline"><i class="bi bi-telephone-x-fill"></i></span><span class="call-accept"><i class="bi bi-telephone-fill"></i></span></div></div><div class="upi-card"><div class="upi-card-head"><i class="bi bi-arrow-down-circle-fill"></i><div><strong>Collect request</strong><span class="small text-muted">from fraud.refund@upi</span></div></div><div class="upi-amount clue" data-clue="${scenario.clues[2]}">\u20b9 24,999</div><p class="small clue" data-clue="${scenario.clues[3]}">Approve within 2 minutes or this refund will be cancelled</p><div class="upi-pin-row">${'\u25cf\u25cf\u25cf\u25cf'.split('').map(()=>'<span></span>').join('')}</div><div class="upi-actions"><button type="button">Decline</button><button type="button" class="upi-approve">Enter UPI PIN</button></div></div></div>`;

    if (id === 'oauth-consent') return `${safety}<div class="login-window"><div class="browser-tabs"><div class="browser-dots"><span></span><span></span><span></span></div><div class="browser-tab"><i class="bi bi-shield-check"></i> Sign in to continue <i class="bi bi-x"></i></div></div><div class="browser-controls"><i class="bi bi-arrow-left"></i><i class="bi bi-arrow-clockwise"></i><div class="address-pill"><i class="bi bi-lock-fill me-2"></i>https://login.microsoftonline.com/oauth2/consent</div><i class="bi bi-star"></i></div><div class="consent-stage"><div class="consent-card"><div class="consent-app"><span class="consent-app-icon"><i class="bi bi-file-earmark-text"></i></span><div><strong>SuperDocs Editor</strong><div class="small ${''}"><span class="clue" data-clue="${scenario.clues[1]}">Unverified publisher &middot; registered 4 days ago</span></div></div></div><p class="small text-muted">This app would like to:</p><ul class="consent-scopes"><li><i class="bi bi-check2"></i> <span class="clue" data-clue="${scenario.clues[2]}">Read and send mail as you</span></li><li><i class="bi bi-check2"></i> <span class="clue" data-clue="${scenario.clues[0]}">Read your full contact list</span></li><li><i class="bi bi-check2"></i> Maintain access to data you have given it access to</li><li><i class="bi bi-check2"></i> Read your basic profile</li></ul><p class="small text-muted clue" data-clue="${scenario.clues[3]}">Link opened from an unsolicited shared-document message</p><div class="consent-actions"><button type="button">Cancel</button><button type="button" class="consent-accept">Accept</button></div></div></div></div>`;
