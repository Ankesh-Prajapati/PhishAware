document.addEventListener('pa:ready', () => {
  ...
  // "Continue learning" prioritizes scenarios with no attempt yet, in
  // catalog order, so it actually reflects progress instead of always
  // showing the same first 3 entries regardless of what's been done. Once
  // every scenario has at least one attempt, it falls back to the 3 with
  // the lowest best score - a "practice your weak spots" suggestion instead
  // of going stale.
  const notAttempted = PA_SCENARIOS.filter(item => !state.attempts?.[item.id]);
  const continueItems = notAttempted.length
    ? notAttempted.slice(0, 3)
    : [...PA_SCENARIOS].sort((a, b) => (state.attempts?.[a.id]?.bestScore ?? state.attempts?.[a.id]?.score ?? 0) - (state.attempts?.[b.id]?.bestScore ?? state.attempts?.[b.id]?.score ?? 0)).slice(0, 3);
  ...
  <div class="col-lg-7"><div class="card p-4"><div class="d-flex justify-content-between mb-3"><h2 class="h5 mb-0">Continue learning</h2><a href="simulations.html" class="small">View all</a></div>${continueItems.map(item => { const attempt = state.attempts?.[item.id]; return `<div class="d-flex align-items-center gap-3 py-3 border-bottom"><div class="scenario-icon"><i class="bi ${item.icon}"></i></div><div class="flex-grow-1"><div class="fw-semibold">${item.title}</div><div class="small text-muted">${attempt ? `Best score ${attempt.bestScore ?? attempt.score}%` : item.summary}</div></div><a class="btn btn-sm ${attempt ? 'btn-soft' : 'btn-primary'}" href="${item.page}">${attempt ? 'Retry' : 'Start'}</a></div>`; }).join('')}</div></div>
